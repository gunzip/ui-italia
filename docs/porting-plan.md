...
Prossimo: **W2 — primitive core quasi completo.** Importate dal registry Base UI tutte le primitive (`button`, `alert`, `badge`, `card`, `input`, `textarea`, `label`, `field`, `input-group`, `checkbox`, `radio-group`, `switch`, `select`, `native-select`, `combobox`, `command`, `separator`, `skeleton`, `spinner`, `tabs`, `tooltip`, `popover`, `dialog`, `alert-dialog`, `sheet`, `drawer`, `dropdown-menu`, `context-menu`, `menubar`, `navigation-menu`, `pagination`, `breadcrumb`, `avatar`, `progress`, `scroll-area`, `slider`, `toggle`, `toggle-group`, `table`, `calendar`, `collapsible`, `accordion`, `aspect-ratio`, `button-group`, `empty`, `hover-card`, `item`, `kbd`, `marker`, `resizable`, `sonner`, `carousel`, `chart`, `sidebar`, `bubble`, `direction`) con **54 file di story** e gate a11y verde (**213 test**, 56 file).

Fine-tuning Italia fatto: Alert (bordo sinistro 4px + varianti status), Input/Textarea (48px, radius 6px, focus 2px), Card (radius 8px + elevazione), Tooltip (`#455B71`), Badge (varianti status), overlay modali e hover del bottone tokenizzati; fix reali a `slider` (thumb singolo per valore scalare) e rimozione colori hard-coded.

Restano in W2: registry item per-componente (oggi il registry espone `theme`/`utils`/`button`) e polish visivo puntuale; poi **W3** (RHF+Zod, date picker), **W4/W5** (blocchi: Stepper, Wizard, Timeline, Header/Footer, Party/Product…), **W4** residui e **W6** (illustrazioni/icone/asset).
