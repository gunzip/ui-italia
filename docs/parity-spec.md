...
| Composizioni            | 🟡 parziale: portate `BoxedModule` (+ `BoxedModuleTitle`) e `Timeline` (+ `TimelineItem`) in `packages/ui/src/blocks/` con story `Blocks/*`. Restano Header/Footer/Banner/Hero/Party*/Wizard ecc.                                                                                                                                                                                                                                     |
...
- **F4 — Catalogo: in corso.** Onde 1-4: icone, illustrazioni, assets, `BoxedModule`, `Timeline`, `Stepper`, `Tag`/`TagGroup`. Onda 5: `CopyToClipboardButton`, `TOSAgreement`, `SingleFileInput`. Onda 6: `Wizard`, `SpidSelectOIDialog` (MI* completati). Restano le composizioni Header/Footer/Banner/Hero/Party* e registry/MDX.
