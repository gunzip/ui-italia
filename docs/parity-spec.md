...
| filled (MIAlert) | bg `{status}[100]`, bordo pieno `{status}[500]`, icona/testo `{status}[850]`, radius 8, p 16 | ✅ `appearance="filled"`                 | 🟢 (F3) |
...
### 5.15 SpidSelectOIDialog

> Blocco in `src/blocks/spid-select-dialog.tsx`, port di `MISpidSelectOIDialog`.
> Oracolo: **`themeNext`**. ⚠️ Lo Storybook di `mui-italia` parte con il tema **legacy**
> (`theme.ts`): senza commutare il global theme su `next` il confronto mostra differenze di
> _generazione_ (radius, backdrop, primary, divider), non di porting.

| Aspetto             | Target (mui, `themeNext`)                                 | ui-italia                                                          | Gap     |
| ------------------- | --------------------------------------------------------- | ------------------------------------------------------------------ | ------- |
| backdrop            | `rgba(14,15,19,.35)`                                      | ✅ `--overlay`                                                     | 🟢      |
| paper radius        | 8                                                         | ✅ `rounded-lg`                                                    | 🟢      |
| larghezza           | `410px` (`sm`) / `600px` (`lg`); fullscreen sotto `sm`    | ✅ fullscreen < `sm`; box `458`/`640` (item `410`/`592`)           | 🟢 (F2) |
| padding contenuto   | 24                                                        | ✅ `p-6`                                                           | 🟢      |
| titolo              | 18 (xs) → 24 (sm) / 700 / lh 1.5                          | ✅ `text-[18px] sm:text-[24px] font-bold leading-[1.5]`            | 🟢 (F1) |
| close               | colore `#0E0F13`, icona ~20–21                            | ✅ `text-foreground` (icona 20)                                    | 🟢 (F1) |
| riga IDP            | h 60, p 16, radius 8, `mb 8`                              | ✅ `h-[60px] rounded-lg mb-2`                                      | 🟢 (F2) |
| label               | 14 / 500, letter-spacing `normal`                         | ✅ 14 / 500, letter-spacing `normal`                               | 🟢 (F2) |
| logo                | h 28, larghezza auto (~115)                               | ✅ `h-7 w-auto object-contain`                                     | 🟢 (F1) |
| skeleton            | riga bordata 60 + barra 240×16                            | ✅ riga 60 bordata + barra 240×16                                  | 🟢 (F2) |
| errore: margine     | `my` 64                                                   | ✅ `my-16`                                                         | 🟢 (F2) |
| alert IDP non disp. | `MIAlert` filled (bg warning[100], bordo warning[500])    | ✅ `Alert appearance="filled"` + icona 24                          | 🟢 (F2) |
| max-height / scroll | `maxHeight: calc(100% - 64px)`, `DialogContent` scrolla   | ✅ `sm:max-h-[calc(100dvh-4rem)] overflow-y-auto` + `tabIndex={0}` | 🟢 (F3) |
| test hook           | id riga, `data-testid` logo/spinner/skeleton/alert/errore | ✅ stessi hook di `mui-italia`                                     | 🟢 (F3) |
| transizione         | `transitionDuration={0}`                                  | zoom/fade 100ms (test visuali con `animations: disabled`)          | 🟡      |
| shuffle + scrollTop | `shuffleList` + `scrollTo({ top: 0 })`                    | ✅ shuffle deterministico + `scrollTo(top)`                        | 🟢 (F3) |

> Note F2:
>
> - **Mobile**: in `mui-italia` il `DialogContent` fullscreen ha `width:100%` in content-box, quindi
>   l'area item deborda (`390px` item su viewport `390`, close button tagliato). ui-italia usa il
>   fullscreen corretto (paper = viewport, gutter `24px`): il bug **non** viene replicato.
> - **Larghezze**: `mui-italia` rende box `458px` (`sm`) / `640px` (`lg`) perché i `width 410/600`
>   sono content-box; ui-italia usa border-box con outer `458`/`640`, così l'area item è `410` (`sm`)
>   e `592` (`lg`, `8px` in meno per via del cap `640`). Differenza residua trascurabile.
>
> Note F3:
>
> - **Shuffle deterministico** (deviazione consapevole): `mui-italia` usa `Math.random` (ordine
>   diverso a ogni mount, non riproducibile). ui-italia usa un Fisher–Yates con **seed fisso**: l'ordine
>   resta non alfabetico ma stabile, così le baseline visuali non sono flaky.
> - `scrollTo({ top: 0 })` invariato rispetto a mui; il contenitore scrollabile riceve `tabIndex={0}`
>   per soddisfare `scrollable-region-focusable` (regione senza contenuto focusable durante l'auth).
> - **Test hook** allineati a mui (`id="spid-select-{entityID}"`, `data-testid` per logo/spinner/
>   skeleton/alert/error state) per facilitare la migrazione dei test a valle.

Fonte: `MISpidSelectOIDialog.tsx`, `SpidList.tsx`, `ErrorState.tsx`, `utils.ts`; misure via `getComputedStyle` (Playwright).
Task di chiusura: `porting-followup.md` §1b. Contratto automatico: `Foundations/Parity guard` + `play` delle story.
...
| `Components/MISpidSelectOIDialog`                                                                                                                                                                             | `src/stories/MISpidSelectOIDialog.stories.tsx` | Default, Loading, UnavailableIdp, Authorizing, CustomTranslations, Interactive, NoIdpsError | 🟢       |
...
| Componenti MI* mancanti | ✅ completati: portati in `packages/ui/src/blocks/` `Stepper`, `Tag`/`TagGroup`, `BoxedModule`, `Timeline`, `CopyToClipboardButton`, `TOSAgreement`, `SingleFileInput`, `Wizard`/`WizardStep`, `SpidSelectOIDialog`. Mappati su primitive esistenti: `MIPaper`→Card, `MIChip`→Badge, `MIIconButton`→Button, `MISnackbar`→Sonner, `MIBreadcrumbs`→Breadcrumb, `CodeInput`→InputOTP, `Autocomplete`→Combobox, `ButtonNaked`→Button link                                                                                                                                                                           |
| Composizioni            | 🟡 parziale: portate in `packages/ui/src/blocks/` `BoxedModule`, `Timeline`, `Stepper`, `Tag`/`TagGroup`, `CopyToClipboardButton`, `TOSAgreement`, `SingleFileInput`, `Wizard`, `SpidSelectOIDialog`, `EnvironmentBanner`, `LangSwitch`, `HorizontalNav`, `Banner`, `PartyAvatar`, `ProductAvatar`, `ProfileItem`, `Footer`/`FooterLegal`/`FooterCheckout`/`FooterPostLogin`/`FooterPreLogin`, `Hero`, `PartyAccountItem`/`PartyAccountItemButton`, `ProductSwitch`, `AccountDropdown`, `Showcase`, `Infoblock`, `Walkthrough`, `PartySwitch`, `HeaderAccount`, `HeaderProduct`. Catalogo composizioni completo |
| Registry                | ✅ 89 item generati (componenti, blocchi, icone, illustrazioni, assets) + `registry:build` verde                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |     |
...
- **F4 — Catalogo: in corso.** MI* completati. Composizioni: onde 7-10 (`EnvironmentBanner`, `LangSwitch`, `HorizontalNav`, `Banner`, `PartyAvatar`, `ProductAvatar`, `ProfileItem`, famiglia `Footer`, `Hero`, `PartyAccountItem`/`PartyAccountItemButton`). Catalogo composizioni completo. Registry fatto; MDX: aggiunta guida `Guides/Parity` (restano i doc per-componente).
