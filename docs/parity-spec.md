...
| Composizioni            | 🟡 parziale: portate in `packages/ui/src/blocks/` `BoxedModule`, `Timeline`, `Stepper`, `Tag`/`TagGroup`, `CopyToClipboardButton`, `TOSAgreement`, `SingleFileInput`, `Wizard`, `SpidSelectOIDialog`, `EnvironmentBanner`, `LangSwitch`, `HorizontalNav` con story `Blocks/*`. Restano `Banner`, `Hero`, `Infoblock`, `Footer` (5), `HeaderAccount`/`HeaderProduct`, `Party*`/`Product*`, `ProfileItem`, `AccountDropdown`, `Showcase`, `Walkthrough` |
...
- **F4 — Catalogo: in corso.** MI* completati (onde 1-6). Onda 7: composizioni leggere (`EnvironmentBanner`, `LangSwitch`, `HorizontalNav`). Restano `Banner`, `Hero`, `Infoblock`, Footer/Header, `Party*`/`Product*`, `ProfileItem`, `AccountDropdown`, `Showcase`, `Walkthrough`, poi registry/MDX.
