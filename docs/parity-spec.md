...
| Composizioni            | 🟡 parziale: portate in `packages/ui/src/blocks/` `BoxedModule`, `Timeline`, `Stepper`, `Tag`/`TagGroup`, `CopyToClipboardButton`, `TOSAgreement`, `SingleFileInput`, `Wizard`, `SpidSelectOIDialog`, `EnvironmentBanner`, `LangSwitch`, `HorizontalNav`, `Banner`, `PartyAvatar`, `ProductAvatar`, `ProfileItem`, `Footer`/`FooterLegal`/`FooterCheckout`/`FooterPostLogin`/`FooterPreLogin`, `Hero`, `PartyAccountItem`/`PartyAccountItemButton`, `ProductSwitch`, `AccountDropdown`, `Showcase`. Restano `Infoblock`, `HeaderAccount`/`HeaderProduct`, `Walkthrough` |
| Registry                | ✅ 84 item generati (componenti, blocchi, icone, illustrazioni, assets) + `registry:build` verde                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |     |
...
- **F4 — Catalogo: in corso.** MI* completati. Composizioni: onde 7-10 (`EnvironmentBanner`, `LangSwitch`, `HorizontalNav`, `Banner`, `PartyAvatar`, `ProductAvatar`, `ProfileItem`, famiglia `Footer`, `Hero`, `PartyAccountItem`/`PartyAccountItemButton`). Restano `Infoblock`, Header, `Walkthrough`. Registry fatto; MDX: aggiunta guida `Guides/Parity` (restano i doc per-componente).
