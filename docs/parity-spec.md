...
| Componenti MI* mancanti | 🟡 parziale: portati `Stepper` (+ `Tag`/`TagGroup`) in `packages/ui/src/blocks/`. Mappati su primitive esistenti: `MIPaper`→Card, `MIChip`→Badge, `MIIconButton`→Button, `MISnackbar`→Sonner, `MIBreadcrumbs`→Breadcrumb, `CodeInput`→InputOTP, `Autocomplete`→Combobox, `ButtonNaked`→Button link. Restano: `Wizard`, `MIBoxedModule` (fatto), `MISpidSelectOIDialog`, `SingleFileInput`, `CopyToClipboardButton`, `TOSAgreement` |
...
- **F4 — Catalogo: in corso.** Onda 1: icone (8) + illustrazioni (97). Onda 2: assets (6). Onda 3: `BoxedModule`, `Timeline`. Onda 4: `Stepper`, `Tag`/`TagGroup`. Restano Wizard/SpidDialog, SingleFileInput/CopyToClipboard/TOS, e le composizioni Header/Footer/Banner/Hero/Party*.
