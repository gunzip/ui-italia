...
| Componenti MI* mancanti | 🟡 parziale: portati `Stepper`, `Tag`/`TagGroup`, `CopyToClipboardButton`, `TOSAgreement`, `SingleFileInput` in `packages/ui/src/blocks/`. Mappati su primitive esistenti: `MIPaper`→Card, `MIChip`→Badge, `MIIconButton`→Button, `MISnackbar`→Sonner, `MIBreadcrumbs`→Breadcrumb, `CodeInput`→InputOTP, `Autocomplete`→Combobox, `ButtonNaked`→Button link. Restano: `Wizard`, `MISpidSelectOIDialog` |
...
- **F4 — Catalogo: in corso.** Onda 1: icone + illustrazioni. Onda 2: assets. Onda 3: `BoxedModule`, `Timeline`. Onda 4: `Stepper`, `Tag`/`TagGroup`. Onda 5: `CopyToClipboardButton`, `TOSAgreement`, `SingleFileInput`. Restano `Wizard`/`SpidDialog` e le composizioni Header/Footer/Banner/Hero/Party*.
