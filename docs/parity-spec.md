...
| Composizioni            | 🟡 parziale: portate in `packages/ui/src/blocks/` `BoxedModule`, `Timeline`, `Stepper`, `Tag`/`TagGroup`, `CopyToClipboardButton`, `TOSAgreement`, `SingleFileInput`, `Wizard`, `SpidSelectOIDialog`, `EnvironmentBanner`, `LangSwitch`, `HorizontalNav`, `Banner`, `PartyAvatar`, `ProductAvatar`, `ProfileItem`, `Footer`/`FooterLegal`/`FooterCheckout`/`FooterPostLogin`/`FooterPreLogin`, `Hero`, `PartyAccountItem`/`PartyAccountItemButton`. Restano `Infoblock`, `HeaderAccount`/`HeaderProduct`, `ProductSwitch`, `AccountDropdown`, `Showcase`, `Walkthrough` |
...
- **F4 — Catalogo: in corso.** MI* completati. Composizioni: onde 7-10 (`EnvironmentBanner`, `LangSwitch`, `HorizontalNav`, `Banner`, `PartyAvatar`, `ProductAvatar`, `ProfileItem`, famiglia `Footer`, `Hero`, `PartyAccountItem`/`PartyAccountItemButton`). Restano `Infoblock`, Header, `ProductSwitch`, `AccountDropdown`, `Showcase`, `Walkthrough`, poi registry/MDX.
