...
| Composizioni            | 🟡 parziale: portate in `packages/ui/src/blocks/` `BoxedModule`, `Timeline`, `Stepper`, `Tag`/`TagGroup`, `CopyToClipboardButton`, `TOSAgreement`, `SingleFileInput`, `Wizard`, `SpidSelectOIDialog`, `EnvironmentBanner`, `LangSwitch`, `HorizontalNav`, `Banner`, `PartyAvatar`, `ProductAvatar`, `ProfileItem` con story `Blocks/*`. Restano `Hero`, `Infoblock`, `Footer` (5), `HeaderAccount`/`HeaderProduct`, `PartyAccountItem`/`PartyAccountItemButton`/`ProductSwitch`, `AccountDropdown`, `Showcase`, `Walkthrough` |
...
- **F4 — Catalogo: in corso.** MI* completati. Composizioni: onde 7-8 (`EnvironmentBanner`, `LangSwitch`, `HorizontalNav`, `Banner`, `PartyAvatar`, `ProductAvatar`, `ProfileItem`). Restano `Hero`, `Infoblock`, Footer/Header, `Party*Item`/`ProductSwitch`, `AccountDropdown`, `Showcase`, `Walkthrough`, poi registry/MDX.
