...
| Composizioni            | 🟡 parziale: portate in `packages/ui/src/blocks/` `BoxedModule`, `Timeline`, `Stepper`, `Tag`/`TagGroup`, `CopyToClipboardButton`, `TOSAgreement`, `SingleFileInput`, `Wizard`, `SpidSelectOIDialog`, `EnvironmentBanner`, `LangSwitch`, `HorizontalNav`, `Banner`, `PartyAvatar`, `ProductAvatar`, `ProfileItem`, `Footer`/`FooterLegal`/`FooterCheckout`/`FooterPostLogin`/`FooterPreLogin`. Restano `Hero`, `Infoblock`, `HeaderAccount`/`HeaderProduct`, `PartyAccountItem`/`PartyAccountItemButton`/`ProductSwitch`, `AccountDropdown`, `Showcase`, `Walkthrough` |
...
- **F4 — Catalogo: in corso.** MI* completati. Composizioni: onde 7-9 (`EnvironmentBanner`, `LangSwitch`, `HorizontalNav`, `Banner`, `PartyAvatar`, `ProductAvatar`, `ProfileItem`, famiglia `Footer`). Restano `Hero`, `Infoblock`, Header, `Party*Item`/`ProductSwitch`, `AccountDropdown`, `Showcase`, `Walkthrough`, poi registry/MDX.
